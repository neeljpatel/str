...
import { Home, Image as ImageIcon, BarChart3, Tag, Lock, LogOut, ChevronDown, ChevronRight } from 'lucide-react';
...
export default function Admin2Layout({ children }: { children: React.ReactNode }) {
  ...
  const [propertyList, setPropertyList] = useState<{slug: string, title: string}[]>([]);
  const supabase = createClient();

  useEffect(() => {
    getProperties().then(props => {
      setPropertyList(Object.keys(props).map(slug => ({
        slug,
  ...
  const handleLogout = async () => {
    ...
    router.refresh(); // Crucial to update the middleware check immediately natively
  };
    ...
    <div className="flex bg-[#f5f5f9] min-h-screen font-sans text-[#566a7f]">
      ...
      <aside className="w-[260px] bg-white flex flex-col shrink-0 border-r border-[#eceef1] shadow-[0_0.125rem_0.375rem_0_rgba(161,172,184,0.12)] z-20">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-6 scrollbar-thin">
          ...
          {/* Section: Security */}
          <div className="mb-2 mt-4 pt-4 border-t border-[#eceef1]">
            <NavItem 
              href="/admin/security" 
              icon={Lock} 
              label="Security" 
